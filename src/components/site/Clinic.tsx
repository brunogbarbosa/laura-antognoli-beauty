...
import { clinicPhotos } from "./clinic";
import { WHATSAPP } from "./data";

export function Clinic() {
  const [index, setIndex] = useState<number | null>(null);
  const open = index !== null;
  const active = open ? clinicPhotos[index] : null;

  const prev = useCallback(
    () => setIndex((i) => (i === null ? i : (i - 1 + clinicPhotos.length) % clinicPhotos.length)),
    [],
  );
  const next = useCallback(
    () => setIndex((i) => (i === null ? i : (i + 1) % clinicPhotos.length)),
    [],
  );
  ...
  return (
    <section id="clinica" className="relative bg-ivory py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-5 md:px-10">
        <Reveal className="text-center">
          <p className="label-caps text-rosegold">O Espaço</p>
          <h2 className="mx-auto mt-4 max-w-2xl font-display text-4xl font-light italic leading-tight text-wine md:text-5xl">
            A Clínica Dra. Laura Antognoli
          </h2>
          <p className="mx-auto mt-5 max-w-xl text-sm font-light leading-relaxed text-taupe">
            Um ambiente pensado nos mínimos detalhes — mármore, ouro e tons vinho
            para uma experiência de luxo a cada visita. Toque em uma foto para ampliar.
          </p>
      ...
      <Dialog open={open} onOpenChange={(o) => !o && setIndex(null)}>
        <DialogContent className="max-w-5xl gap-0 overflow-hidden border-border bg-ink p-0">
          {active && (
            <>
